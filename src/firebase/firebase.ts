...
class Firebase {
	...
	public searchProducts = (searchKey: string) => {
		let didTimeout = false;
		...
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				clearTimeout(timeout);
				if (!didTimeout) {
					let searchedKeywordsProducts: Partial<IProduct>[] = [];
					let searchedNameProducts: Partial<IProduct>[] = [];
					let lastKey: any = null;

					if (!nameSnaps.empty) {
						nameSnaps.forEach((doc) => {
							searchedNameProducts = searchedNameProducts.concat([{ id: doc.id, ...doc.data() }]);
						});
					...
					if (!keywordsSnaps.empty) {
						keywordsSnaps.forEach((doc) => {
							searchedKeywordsProducts = searchedKeywordsProducts.concat([{ id: doc.id, ...doc.data() }]);
						});
					}
					...
					mergedProducts.forEach((product: IProduct) => {
